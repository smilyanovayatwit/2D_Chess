...
public class ChessClient2 extends ChessBoard
    {
    ...
    public static void main( final String argv[] ) throws Exception
        ...
        final MouseListener mlistener = new MouseAdapter()
            ...
            public void mousePressed( final MouseEvent e )
                ...
                if ( !ChessClient2.hasWon )
                    ...
                    if ( ( ( x > 60 ) && ( x < 430 ) ) &&
                         ( ( y > 60 ) && ( y < 430 ) ) )
                        {
                        ChessClient2.startRow = findWhichTileSelected( y ) ;
                        ChessClient2.startColumn = findWhichTileSelected( x ) ;
                        final int piece = chessWindow.mainChessBoard.cellMatrix.getPieceCell( ChessClient2.startRow,
                                                                                              ChessClient2.startColumn ) ;
            ...
            public void mouseReleased( final MouseEvent e )
                ...
                final int x = e.getX() ;
                final int y = e.getY() ;
                ChessClient2.endRow = findWhichTileSelected( y ) ;
                ChessClient2.endColumn = findWhichTileSelected( x ) ;
                final int p = chessWindow.mainChessBoard.cellMatrix.getPieceCell( ChessClient2.endRow,
                                                                                  ChessClient2.endColumn ) ;

                // sends END message to the server with the coordinates and the chess
                // piece
                try
                    {
                    outToServer.writeBytes( "END" + " " + ChessClient2.endRow + " " +
                                            ChessClient2.endColumn + " " + p +
                                            "\r\n" ) ;
                    }
                catch ( final IOException e1 )
                    {
